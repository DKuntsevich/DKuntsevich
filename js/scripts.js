...
$(window).load(function(){
	var win=$(window)
		...
		,nav=$('nav')
		,content=$('#content')
		,_scrollable=win._scrollable()
		,nav_opt=nav.data('navs')
		,cords=($.map(content.find('>ul>li'),function(e){return e.offsetTop}))
		,fst=true
	...
	$('.open-by-overlay')
		.click(function(){
			...
			bg.add('.popup a.close').click(function(){
	...
	win.on('scroll',function(e,d){
		var pos=_scrollable.prop('scrollTop')
			,nav_n=nav.data('navs').n
			,scroll_n=(function(){
				for(var i=0,l=cords.length;i<l;i++)				
					if(pos<=cords[i])
						return i
			})()
		if(scroll_n!=nav_n)			
	...
	content
		.tabs({
			...
			,actFu:function(_){
				_scrollable.stop()
				if(_.curr)
					$.scrollTo(_.curr,{duration:800,axis:'y',easing:'easeInOutSine'})
			}
		})
	nav
		.navs({
	...
	$('>ul>li a',nav).click(function(){		
		content.tabs('#!/'+this.href.replace(/.+[#!/]+(.+)/,'$1'))
	})
	...
	win.resize(function(){
		_scrollable.stop()
		var curr=content.data('tabs')
		if(curr)
			$.scrollTo(curr,{duration:800,axis:'y',easing:'easeInOutSine'})		
	})
